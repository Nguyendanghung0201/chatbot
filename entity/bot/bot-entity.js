...
module.exports = {
    ...
    getBotNameByUser: async (bot) => {
        return global.db.select().table(table).where('user_id', bot.user_id).andWhere('botname', bot.botname).first()
    },
    ...
    getBotById: async (user_id, bot_id) => {
        return global.db.select('id', 'botname', 'botaction_id').table(table).where('user_id', user_id).andWhere('id', bot_id).first()
    },
    ...
    getAllBotByUser: async (user_id) => {
        return global.db(table).select('id', 'botname', 'botaction_id').where('user_id', user_id)
    }
}